<div class="row" style="margin: 0 auto">
  <div class="col l5 hide-on-med-and-down" id="image-fixed">
    <img src="/assets/img/slider/eeepjaa.webp" alt="EEEPJAA">
  </div>
  <div class="col l7 s12" style="padding: 0">
    <div class="carousel carousel-slider center slider" data-indicators="true" id="slider">
      <div class="carousel-fixed-item space-between">
        <button id="carousel-prev-button" class="btn-floating waves-effect btn-medium">
          <i class="material-icons">arrow_back</i>
        </button>
        <button id="carousel-next-button" class="btn-floating waves-effect btn-medium">
          <i class="material-icons">arrow_forward</i>
        </button>
      </div>
      <div class="carousel-item white-text">
        <img src="/assets/img/slider/sisu2019.webp" alt="Sisu 2019" class="slider-image">
      </div>
      <div class="carousel-item white-text" id="last-slide">
        <img src="/assets/img/home/enfermagem.webp" alt="Sisu 2019" class="slider-image">
      </div>
    </div>
  </div>
</div>